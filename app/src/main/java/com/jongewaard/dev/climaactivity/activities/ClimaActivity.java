...
import com.jongewaard.dev.climaactivity.adapters.ClimaAdapter;
import com.jongewaard.dev.climaactivity.data.ClimaDataAsyncTask;
import com.jongewaard.dev.climaactivity.location.LocationDetector;
...
public class ClimaActivity extends AppCompatActivity implements ClimaDataAsyncTask.IAsyncTaskResponder {

    public static final String TAG = ClimaActivity.class.getSimpleName();
    ...
    private Clima[] mWeatherModel;
    private ClimaDataAsyncTask mWeatherDataAsyncTask;
...
//        mIconImageView = (ImageView)findViewById(R.id.iconImageView);
    ...
    public void retrieveWeatherData(){
        ...
        mWeatherDataAsyncTask = new ClimaDataAsyncTask(this); // instantiates WeatherDataAsyncTask
    ...
    @Override
    public void asyncTaskFinished(Clima[] weatherForecast) {

        // initialize the array of Weather objects to be the length of the input arg array
        mWeatherModel = new Clima[weatherForecast.length];

        int i = 0;
        for(Clima w : weatherForecast){ // copy data from weatherForecast into mWeatherModel
            mWeatherModel[i] = w;
            i++;
        }
