package com.jongewaard.dev.climaactivity.data;
...
import com.jongewaard.dev.climaactivity.BuildConfig;
...
* */
    public interface IAsyncTaskResponder {
        void asyncTaskFinished(Clima[] weatherForecast);
    }

    public IAsyncTaskResponder mIAsyncTaskListener = null;

    public ClimaDataAsyncTask(IAsyncTaskResponder responder){
        this.mIAsyncTaskListener = responder;
    }

    // Tag for error logging in logcat
    private final String LOG_TAG = ClimaDataAsyncTask.class.getSimpleName();
...
//                        .build();
        ...
        try{
            Clima[] Clima = formatJsonArray(jsonData, howManyDays);
            return Clima;
...
//            String[] formattedData = new String[howManyDays]; // will be returned from this method
        ...
        Clima[] ClimaForecast = new Clima[jsonArray.length()]; // model object
        ...
        for(int i=0; i<jsonArray.length(); i++){
            ...
            Clima Clima = new Clima( // calls constructor for model data Weather object
                    formattedStr,
